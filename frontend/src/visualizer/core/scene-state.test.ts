...
describe('canonical scene state', () => {
    ...
    test('accepts a material trail beside the canonical state loop', () => {
        // The canonical form constrains the state, not the material: exactly one previous-frame
        // read of the combine output, with plugin trails legal beside it. The earlier rule —
        // exactly one previous-frame image edge in the whole scene — is what made every stage
        // upstream of the combine a fresh redraw.
        const scene = firstLightScene(createM1Registry());
