...
describe('scenes accumulate and move', () => {
    ...
    test('every family reads its previous frame at least through the canonical state', () => {
        ...
        for (const theme of THEMES) {
            ...
            // to the family's own budget plus the state (ADR-0016).
            for (const scene of scenes) {
...
/**
 * `TemporalTransform` (spec section 19.8): the catalog's history-port transformer, whose `decay`
 * is the gain of any cycle closing on it (ADR-0016). Coverage updated to the sampler-level
 * temporal identity: a historical input is a `previousTexture` read, not a raw resource id.
 */
