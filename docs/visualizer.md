...
fresh material. The combine output is the only active layer presented by the host. The combine's
arithmetic is drawn per scene from an operator family — stamp, screen, or max (ADR-0016). Beneath
the state, material memory is legal and bounded: plugins may keep self-loop trails and wiring may
fold the composed image back through a lossy port, with every image cycle required to converge at
its gain ceiling before the scene compiles.
